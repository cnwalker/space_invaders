...
Game::Game(int num_invaders, int num_invaders_per_row, int screen_width, int screen_height) {
  int player_width = 50;
  int player_height = 50;
  player = new Character(screen_width / 2, screen_height - player_height - 20,
                         player_width, player_height, 3);

  int invader_width = (screen_width / 2) / num_invaders_per_row;
  int invader_height = invader_width;
  int next_start_x = invader_width / 2;
  int next_start_y = invader_height / 2;
  int invader_margin = invader_width / 4;
 
  for (int cur_invaders = 1; cur_invaders <= num_invaders; cur_invaders++) {
    Character *invader =
        new Character(next_start_x, next_start_y, invader_width, invader_height, 1);
    invaders.push_back(invader);
    next_start_x += invader_width + invader_margin;
    if (cur_invaders % num_invaders_per_row == 0) {
